...
/* =========================================================
   THE DROWNED CROWN
   READER + NARRATION 3.4
   DESKTOP WEBGPU + MOBILE WASM
========================================================= */
...
const chapterPOV = document.getElementById("chapterPOV");
const chapterLocation = document.getElementById("chapterLocation");
const chapterText = document.getElementById("chapterText");
const headerChapterTitle = document.getElementById("headerChapterTitle");

const previousChapter = document.getElementById("previousChapter");
const previousChapterName = document.getElementById("previousChapterName");
...
const narrationChapter = document.getElementById("narrationChapter");
const narrationPosition = document.getElementById("narrationPosition");
const narrationSeek = document.getElementById("narrationSeek");
const narrationSeekFill = document.getElementById("narrationSeekFill");

const previousParagraph = document.getElementById("previousParagraph");
const playPause = document.getElementById("playPause");
const nextParagraph = document.getElementById("nextParagraph");
...
function getRequestedChapterId() {
    ...
    return params.get("chapter") || "prologue";
}
...
function getChapterProgressLevel(chapterId) {
...
function saveFurthestProgress(chapterId) {
    const newLevel = getChapterProgressLevel(chapterId);
...
async function loadChapter() {
    const requestedId = getRequestedChapterId();

    const chapter = getChapterById(requestedId);
    ...
    chapterPOV.textContent = chapter.pov;
    chapterLocation.textContent = chapter.location;
    headerChapterTitle.textContent = chapter.label;
...
function prepareChapterText() {
    narrationParagraphs =
        Array.from(
            chapterText.querySelectorAll("p")
        ).filter(
            paragraph =>
...
function restoreNarrationPosition() {
    ...
    if (
        ...
        saved >= 0 &&
        saved < narrationParagraphs.length
    ) {
        currentParagraphIndex = saved;
    } else {
        currentParagraphIndex = 0;
    }
}
...
/* =========================================================
   NARRATION 3.4
   DESKTOP + MOBILE-SAFE DUAL ENGINE
========================================================= */
...
/* =========================================================
   ...
   IMPORTANT:
   Desktop/laptop machines retain WebGPU + FP32.

   Phones/tablets use WASM + Q8 to avoid the large
   WebGPU/FP32 memory spike that can crash mobile browsers.
========================================================= */
...
function detectMobileNarrationDevice() {
    ...
    /*
       ...
       A Mac-like platform with multiple touch points is
       therefore treated as an iPad.

       Windows touchscreen laptops are NOT caught by this.
    */
...
const DROWNED_CROWN_RUNTIME =
    DROWNED_CROWN_IS_MOBILE
        ? {
            ...
            modelCacheVersion:
                "kokoro-82m-v1.0-onnx-q8-wasm-mobile"
        }
        : {
            mode: "desktop",
            device: "webgpu",
...
const MODEL_CACHE_VERSION =
    DROWNED_CROWN_RUNTIME.modelCacheVersion;
...
let drownedCrownGenerationIndex = 0;

let drownedCrownPlaybackChunkIndex = -1;

let drownedCrownGenerationRunning = false;

let drownedCrownAudio = null;
let drownedCrownAudioURL = null;
let drownedCrownPauseTimer = null;

let drownedCrownSessionVoice = "";
let drownedCrownSessionSpeed = 1;
...
function loadNarrationSettings() {
    ...
    if (
        narrationSource &&
        savedSource &&
        Array.from(narrationSource.options)
            .some(
                option =>
...
function splitDrownedCrownParagraph(
    ...
    /*
       Desktop uses the existing narrator-director
       chunk lengths.

       Mobile gets smaller pieces so each Kokoro
       inference requires less working memory.
    */
...
async function startDrownedCrownNarration(
    ...
    /*
       ...
       A cached chunk can be loaded and played without
       loading the neural model at all.
    */
...
async function obtainDrownedCrownChunk(
    ...
    /*
       ...
       No Kokoro model initialization is necessary.
    */
    ...
    */
    ...
    const shouldLoadSilently =
        ...
        drownedCrownPlaybackChunkIndex >= 0;
...
function stopDrownedCrownEngine(
    ...
    drownedCrownGenerationIndex = 0;

    drownedCrownPlaybackChunkIndex =
        -1;

    /*
       We intentionally do NOT destroy Kokoro here.

       On desktop this avoids reloading the large FP32 model.

       On mobile the q8 model remains reusable for the life
       of the page rather than repeatedly allocating it.
    */
}
